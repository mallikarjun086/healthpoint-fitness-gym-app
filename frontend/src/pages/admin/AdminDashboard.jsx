...
import { Dollar3D, Users3D, Shield3D, Chart3D, Wrench3D } from '../../components/ui/Icon3D';
...
const AdminDashboard = () => {
  ...
  const handleToggleEquipment = (id, currentStatus) => {
    ...
    api.post(`/equipment/toggle-status/${id}`, { status: nextStatus })
      .then(() => {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <main className="flex-1 ml-0 md:ml-64 p-6 sm:p-8 relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {statItems.map((stat, i) => (
            <TiltCard 
              key={i} 
              maxTilt={4}
              className="panel-card p-5 space-y-2"
            >
              <div className="flex justify-between items-start">
                ...
                <div className="flex items-center gap-0.5 text-[11px] font-semibold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
        ...
        <TiltCard maxTilt={2} className="panel-card p-6 mb-8 space-y-4">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <span className="caption block">Facility Telemetry</span>
                <h3 className="heading-md text-text-primary">Gym Floor Occupancy</h3>
              </div>
            </div>

            <div className="flex items-center gap-6 w-full md:w-auto justify-between border-t md:border-t-0 border-border pt-3 md:pt-0">
              <div>
                <div className="caption">Active Floor Count</div>
                <div className="text-xl stat-display text-primary">
              ...
              <div className="border-l border-border pl-6">
                <div className="caption">Today's Visits</div>
                <div className="text-xl stat-display text-text-primary">
                  <CountUp value={occupancy.todayTotalCheckIns} />
                </div>
              </div>

              <div className="border-l border-border pl-6 hidden sm:block">
                <div className="caption">Peak Window</div>
                <div className="text-xs font-semibold text-amber-400">{occupancy.peakHours}</div>
              </div>
            </div>
          </div>
        </TiltCard>
        ...
        <section className="panel-card p-6 mb-8 space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="heading-md text-text-primary">Equipment & Zone Status</h3>
                <p className="body-sm text-text-secondary">Track machinery status and operational health</p>
              </div>
            </div>
              ...
              <div key={eq.id} className="p-3.5 rounded-xl bg-surface-elevated border border-border flex items-center justify-between table-row-hover">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-text-muted font-mono">({eq.zone})</span>
                  </div>
            ...
            <div className="h-[240px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={revenueTrendData}>
                  ...
                  <XAxis dataKey="name" stroke="#56575E" tick={{ fontSize: 11, fill: '#8F9098' }} />
                  <YAxis stroke="#56575E" tick={{ fontSize: 11, fill: '#8F9098' }} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#111216', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', color: '#F4F4F6' }} 
                    formatter={(val) => [`₹${val.toLocaleString()}`, 'MRR']}
                  />
                  <Area type="monotone" dataKey="revenue" stroke="#4F46E5" strokeWidth={2.5} fillOpacity={1} fill="url(#revGlow)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </TiltCard>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={revenueTrendData}>
                  ...
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#111216', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', color: '#F4F4F6' }}
                  />
                  <Bar dataKey="members" fill="#4F46E5" radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </TiltCard>
